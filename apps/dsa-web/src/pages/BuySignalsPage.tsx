...
import { TrendingUp, RefreshCw, Search, Filter, Download, X } from 'lucide-react';
...
const BuySignalsPage: React.FC = () => {
  ...
  const signalOptions = [
    ...
    { value: 'B1', label: 'B1 - 左侧买点' },
    { value: 'B2', label: 'B2 - 突破买点' },
    { value: 'B3', label: 'B3 - 回踩买点' },
  ];

  const fetchBuySignals = async () => {
    setIsLoading(true);
    setError(null);

    try {
      const response = await fetch(`/api/v1/signals/buy?date=${selectedDate}`);
      if (!response.ok) {
        throw new Error('获取买入信号失败');
      }
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `buy_signals_${selectedDate}.csv`);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-gray-600 dark:text-gray-400 mt-1">
              当前日期: {selectedDate} | 基于均线三买三卖系统，自动扫描买入信号
            </p>
          </div>
        </div>
      ...
      {error ? (
        ...
        <Card padding="none">
          <DataTable
            ...
            loading={isLoading}
            emptyText="暂无买入信号"
            emptyDescription="当前没有符合条件的买入信号"
