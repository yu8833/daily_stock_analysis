...
import { TrendingUp, RefreshCw, Download, X, Search } from 'lucide-react';
...
const RaceOpenPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const result = await marketDataApi.getRaceOpen(selectedDate);
      setData(result);
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `race_open_${selectedDate}.csv`);
        ...
        <>
          <Card padding="none">
            <DataTable
              ...
              loading={isLoading}
              emptyText="暂无数据"
              emptyDescription={searchKeyword ? '没有找到匹配的早盘抢筹数据' : '没有找到早盘抢筹数据'}
              rowKey={(row) => row.code}
