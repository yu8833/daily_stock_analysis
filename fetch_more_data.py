...
def main():
    ...
    for i, stock in enumerate(selection_data[:100], 1):
        try:
            ...
            if df is not None and hasattr(df, 'empty') and not df.empty:
                ...
                success += 1
                if i % 10 == 0:
                    print(f'  已完成 {i}/{len(selection_data[:100])}，成功 {success}，失败 {fail}')
            else:
                fail += 1
    ...
    for stock in selection_data[:100]:
        daily_data = stock_repo.get_range(stock.code, query_date - date(2026,6,3).timedelta(days=120), query_date)
